...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-out", default=str(Path(__file__).resolve().parents[1] / "models" / "model_week_kr_v2.pt"))
...
def main() -> None:
    ...
    log_path = os.path.join(
        ...
        "log",
        f"model_week_kr_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
    )
