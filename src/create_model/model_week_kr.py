...
def parse_args() -> argparse.Namespace:
    ...
    # 데이터 범위
    parser.add_argument("--table", default="STOCK_DATA_WEEK_KR", help="DB table name")
    ...
    # 체크포인트 및 클래스 불균형
    # checkpoint
    parser.add_argument("--model-out", default=str(Path(__file__).resolve().parents[1] / "models" / "model_week_kr.pt"), help="model output path")
...
def main() -> None:
    ...
    _log_path = os.path.join(
        ...
        "log",
        f"model_week_kr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
    )
