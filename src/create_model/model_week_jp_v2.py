...
V2_MODEL_MODE = "v2_upside_probability_week_jp"
V2_FEATURE_COLS = [
...
def compute_v2_features(raw: np.ndarray) -> np.ndarray:
    ...
    ha_body_ratio = np.clip(
        np.abs(ha_close - ha_open) / (np.abs(ha_high - ha_low) + 1e-10),
        0.0,
        1.0,
    ).astype(np.float32)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-out", default=str(Path(__file__).resolve().parents[1] / "models" / "model_week_jp_v2.pt"))
...
def main() -> None:
    ...
    log_path = os.path.join(
        ...
        "log",
        f"model_week_jp_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
    )
